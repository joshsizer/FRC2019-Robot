...
/**
 * Wrapper class for Joystick, since we must use Joystick and JoystickButton to interface with the
 * Command scheduler.
 * 
 * @author Joshua Sizer
 * @since 1/30/2019
...
public class XboxController {
  ...
  /**
   * Mapping of raw button addresses to usable names. You can verify/change this mapping by plugging
   * in a joystick/controller, moving the sticks/pressing buttons, and the corresponding adress will
   * light up in the driver station. It seems that the axis' are 0-indexed, while the buttons are
   * 1-indexed.
   */
  private enum Mapping {
    kBumperLeft(5), kBumperRight(6), kTriggerLeft(2), kTriggerRight(3), kStickLeftX(0), kStickLeftY(
        1), kStickRightX(4), kStickRightY(5), kA(1), kB(2), kX(3), kY(4), kBack(7), kStart(8);
  ...
  /**
   * Our native Joystick object. We could have this class extend joystick, but I'm trying to keep
   * this as clean as possible.
   */
  Joystick mJoystick;

  /**
   * These are our button objects. With these objects, you can bind commands to specific button
   * presses.
   */
  Button mButtonA, mButtonB, mButtonX, mButtonY, mButtonLBumper, mButtonRBumper, mButtonBack,
      mButtonStart;
  ...
  /**
   * Gets the left stick's X-axis. Pushing right returns a positive value.
   * 
  ...
  /**
   * Gets the left stick's Y-axis. Pushing up returns a positive value. Note that this is slightly
   * different than the default implementation, which by convention for joysticks returns a negative
   * value for up. This convention is stupid, so I am negating here instead of elsewhere.
   * 
  ...
  /**
   * Gets the right stick's X-axis. Pushing right returns a positive value.
   * 
   * @return The right stick's current x-axis value [-1, 1]
   */
  public double getRightXAxis() {
    return mJoystick.getRawAxis(Mapping.kStickRightX.value);
  }

  /**
   * Gets the right stick's Y-axis. Pushing up returns a positive value. Note that this is slightly
   * different than the default implementation, which by convention for joysticks returns a negative
   * value for up. This convention is stupid, so I am negating here instead of elsewhere.
   * 
   ...
   * --------------------------------------------------------------------
   * 
   * This part of the class returns the button object for binding commands to.
   * 
   * --------------------------------------------------------------------
   */

  /**
   * Returns the underlying JoystickButton object associated with this button.
   * 
